...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) fog/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) fog/manage.py$(1)
endif
endef
...
create-db:
	createdb -U fog -l en_US.utf-8 -E utf-8 -O fog -T template0 -e fog

create-db-user:
	createuser -S -d -R -P -e fog

drop-db:
	dropdb -i -e -U fog fog
...
develop:
	...
	pip install -U -e .
	pip install -U -c constraints.txt -r requirements/dev.pip
...
isort:
	isort --recursive setup.py fog/ tests/
...
startapp:
	...
	mkdir -p fog/apps/$$app_name; \
	envdir envs/$(ENV) python manage.py startapp $$app_name fog/apps/$$app_name --template fog/config/app_template; \
	echo "Don't forget to add 'fog.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'fog.config/settings/common.py'!"
...
test-upload:
	...
	python -c "import webbrowser; webbrowser.open('https://testpypi.python.org/pypi/fog')"

upload:
	twine upload -s dist/*
	python -c "import webbrowser; webbrowser.open('https://pypi.python.org/pypi/fog')"
